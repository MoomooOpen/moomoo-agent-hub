...
_VC_EN = (
    ...
    "\n"
    "```bash\n"
    "( _c=\"\"; if command -v {cli} >/dev/null 2>&1; then _c=\"{cli}\"; "
    "elif [ -x \"$HOME/.local/bin/{cli}\" ]; then _c=\"$HOME/.local/bin/{cli}\"; fi; "
    "if [ -n \"$_c\" ]; then \"$_c\" check --quiet --filter cli --filter {slug} 2>/dev/null; "
