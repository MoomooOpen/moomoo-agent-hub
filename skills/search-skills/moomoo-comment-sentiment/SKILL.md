...
description: >-
  ...
  TRIGGER when: 情绪 / 社区情绪 / 散户情绪 / 看多看空 / 多空情绪 / 牛熊 /
  看涨看跌 / 风向 / 讨论氛围 / 评论怎么看 / 大家怎么看 / bullish or bearish /
  sentiment / community mood / retail discussion tone.
  SKIP: news listing → moomoo-news-search; news interpretation/分析/研判 →
...
```bash
( _c=""; if command -v moomoo-skills >/dev/null 2>&1; then _c="moomoo-skills"; elif [ -x "$HOME/.local/bin/moomoo-skills" ]; then _c="$HOME/.local/bin/moomoo-skills"; fi; if [ -n "$_c" ]; then "$_c" check --quiet --filter cli --filter moomoo-comment-sentiment 2>/dev/null; else echo '__CLI_MISSING__'; fi ) > /tmp/.moomoo-comment-sentiment-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.moomoo-comment-sentiment-update 2>/dev/null; rm -f /tmp/.moomoo-comment-sentiment-update
```
...
- **Output containing a line starting with `moomoo-comment-sentiment`** (this skill has an update available): append a notice at the **end** of your reply:
  > `moomoo-comment-sentiment` has a new version available. If you agree, I can upgrade it for you right away.

  If the user agrees, run `moomoo-skills upgrade moomoo-comment-sentiment` to perform the upgrade. (This command also auto-upgrades the `moomoo-skills` manager itself, so a separate CLI prompt is unnecessary when a skill update is available.)
- **Output containing only a line starting with `cli`** (this skill is up-to-date but the `moomoo-skills` manager itself has a new version): append a notice at the **end** of your reply:
  ...
  If both a `cli` line and a `moomoo-comment-sentiment` line appear, only show the skill upgrade prompt above — the manager will be refreshed as a side effect of `moomoo-skills upgrade moomoo-comment-sentiment`.
