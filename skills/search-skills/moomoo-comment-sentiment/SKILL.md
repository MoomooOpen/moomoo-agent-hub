...
```bash
moomoo-skills check --quiet 2>/dev/null | grep '^moomoo-comment-sentiment' > /tmp/.moomoo-comment-sentiment-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.moomoo-comment-sentiment-update 2>/dev/null; rm -f /tmp/.moomoo-comment-sentiment-update
```
...
- **Has content** (this skill has an update available): append a notice at the **end** of your reply:
  ...
  If the user agrees, run `moomoo-skills upgrade moomoo-comment-sentiment` to perform the upgrade.
