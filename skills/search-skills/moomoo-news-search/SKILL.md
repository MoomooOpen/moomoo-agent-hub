...
description: >-
  ...
  TRIGGER when: stock/company + news-intent keyword (新闻 / 消息 / 资讯 / 公告 /
  研报 / 报道 / 头条 / 文章 / news / announcement / research report / headlines /
  latest update). Time qualifiers (最新/最近/近期/今日/latest/recent) + news word
  also trigger (e.g. "腾讯最新新闻", "NVDA latest news") — these are listing
  requests, NOT interpretation.
  SKIP: interpretation verbs (解读/分析/研判/总结/interpret/analyze/summarize) →
  moomoo-stock-digest; community sentiment/评论/讨论/散户情绪 →
  moomoo-comment-sentiment; bare ticker without news keyword;
  price/quote/K线/异动/trade.
metadata:
  version: 0.0.2
  author: moomoo
  requires:
...
```bash
( _c=""; if command -v moomoo-skills >/dev/null 2>&1; then _c="moomoo-skills"; elif [ -x "$HOME/.local/bin/moomoo-skills" ]; then _c="$HOME/.local/bin/moomoo-skills"; fi; if [ -n "$_c" ]; then "$_c" check --quiet --filter cli --filter moomoo-news-search 2>/dev/null; else echo '__CLI_MISSING__'; fi ) > /tmp/.moomoo-news-search-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.moomoo-news-search-update 2>/dev/null; rm -f /tmp/.moomoo-news-search-update
```
