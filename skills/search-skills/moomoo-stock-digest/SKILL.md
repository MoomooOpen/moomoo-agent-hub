...
```bash
( if command -v moomoo-skills >/dev/null 2>&1; then moomoo-skills check --quiet 2>/dev/null | grep -E '^(cli|moomoo-stock-digest)'; else echo '__CLI_MISSING__'; fi ) > /tmp/.moomoo-stock-digest-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.moomoo-stock-digest-update 2>/dev/null; rm -f /tmp/.moomoo-stock-digest-update
```
