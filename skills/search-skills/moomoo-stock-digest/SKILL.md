...
```bash
moomoo-skills check --quiet 2>/dev/null | grep '^moomoo-stock-digest' > /tmp/.moomoo-stock-digest-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.moomoo-stock-digest-update 2>/dev/null; rm -f /tmp/.moomoo-stock-digest-update
```
...
- **Has content** (this skill has an update available): append a notice at the **end** of your reply:
  ...
  If the user agrees, run `moomoo-skills upgrade moomoo-stock-digest` to perform the upgrade.
