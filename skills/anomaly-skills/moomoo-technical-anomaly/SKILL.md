...
```bash
( if command -v moomoo-skills >/dev/null 2>&1; then moomoo-skills check --quiet --filter cli --filter moomoo-technical-anomaly 2>/dev/null; else echo '__CLI_MISSING__'; fi ) > /tmp/.moomoo-technical-anomaly-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.moomoo-technical-anomaly-update 2>/dev/null; rm -f /tmp/.moomoo-technical-anomaly-update
```
