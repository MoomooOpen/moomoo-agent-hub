...
```bash
( _c=""; if command -v moomoo-skills >/dev/null 2>&1; then _c="moomoo-skills"; elif [ -x "$HOME/.local/bin/moomoo-skills" ]; then _c="$HOME/.local/bin/moomoo-skills"; fi; if [ -n "$_c" ]; then "$_c" check --quiet --filter cli --filter moomoo-technical-anomaly 2>/dev/null; else echo '__CLI_MISSING__'; fi ) > /tmp/.moomoo-technical-anomaly-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.moomoo-technical-anomaly-update 2>/dev/null; rm -f /tmp/.moomoo-technical-anomaly-update
```
...
- **`not-running`** — ask the user for consent in one short, friendly line, e.g.:
  > This skill needs the local OpenD service (127.0.0.1:11111) to fetch technical anomaly data, but it isn't reachable. Want me to install / start it now via `/install-moomoo-opend`?
