...
```bash
( _c=""; if command -v moomoo-skills >/dev/null 2>&1; then _c="moomoo-skills"; elif [ -x "$HOME/.local/bin/moomoo-skills" ]; then _c="$HOME/.local/bin/moomoo-skills"; fi; if [ -n "$_c" ]; then "$_c" check --quiet --filter cli --filter moomoo-capital-anomaly 2>/dev/null; else echo '__CLI_MISSING__'; fi ) > /tmp/.moomoo-capital-anomaly-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.moomoo-capital-anomaly-update 2>/dev/null; rm -f /tmp/.moomoo-capital-anomaly-update
```
...
- **Output containing a line starting with `moomoo-capital-anomaly`** (this skill has an update available): append a notice at the **end** of your reply:
  ...
  If the user agrees, run `moomoo-skills upgrade moomoo-capital-anomaly` to perform the upgrade. (This command also auto-upgrades the `moomoo-skills` manager itself, so a separate CLI prompt is unnecessary when a skill update is available.)
- **Output containing only a line starting with `cli`** (this skill is up-to-date but the `moomoo-skills` manager itself has a new version): append a notice at the **end** of your reply:
  ...
  If both a `cli` line and a `moomoo-capital-anomaly` line appear, only show the skill upgrade prompt above — the manager will be refreshed as a side effect of `moomoo-skills upgrade moomoo-capital-anomaly`.
...
- **`not-running`** — ask the user for consent in one short, friendly line, e.g.:
  > This skill needs the local OpenD service (127.0.0.1:11111) to fetch capital anomaly data, but it isn't reachable. Want me to install / start it now via `/install-moomoo-opend`?
