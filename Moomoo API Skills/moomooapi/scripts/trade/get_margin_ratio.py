...
def get_margin_ratio(codes, acc_id=None, market=None, trd_env=None, security_firm=None, ctx_type="SEC", output_json=False):
    acc_id = acc_id or get_default_acc_id()
    trd_env = parse_trd_env(trd_env) if trd_env else get_default_trd_env()

    ctx = None
    try:
        ctx = create_sec_or_future_trade_context(market, security_firm=parse_security_firm(security_firm), ctx_type=ctx_type, codes=codes)
